...
func (ng *rancherNodeGroup) nodePool() *v3.NodePool {
	...
	if err != nil {
		glog.Fatalf("failed to get node pool %s: %s", ng.id, err)
	}
...
func (ng *rancherNodeGroup) MaxSize() int {
	...
	if maxSizeValue, exists := nodePool.Annotations[maxSizeAnnotation]; exists {
		...
		if err != nil {
			glog.Errorf("failed to parse max size from node pool %s: %s", nodePool.HostnamePrefix, err)
...
func (ng *rancherNodeGroup) MinSize() int {
	...
	if minSizeValue, exists := nodePool.Annotations[minSizeAnnotation]; exists {
		...
		if err != nil {
			glog.Errorf("failed to parse min size from node pool %s: %s", nodePool.HostnamePrefix, err)
...
func (ng *rancherNodeGroup) Nodes() ([]string, error) {
...
func (ng *rancherNodeGroup) TemplateNodeInfo() (*cache.NodeInfo, error) {
